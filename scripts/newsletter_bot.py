...
# ── 표준 서식 ──
# 아래 값들은 build_prompt()의 '서식 규칙'과 짝을 이룹니다.
# 한쪽만 고치면 매일 헛경고가 뜨므로 반드시 양쪽을 같이 고치세요.
...
같은 사안을 여러 언론사가 쓴 경우 하나로 합치세요.

### 무엇이 '중요한' 뉴스인가 (선정 우선순위)
이 뉴스레터의 독자는 **금융권 취업을 준비하며 면접을 대비하는 사람**입니다.
"오늘 가장 많이 보도된 것"이 아니라 "이 독자에게 가장 값진 것"을 기준으로 아래 순서로 고르세요.

1순위. 한국 은행·금융지주·금융당국의 **구조적 변화** — 규제 신설·개정, 감독 방향 전환,
   수익구조·사업모델 변화, 대형 M&A·조직개편. 몇 달 뒤 면접장에서도 회자될 뉴스.
2순위. 아래 '거시 트렌드 목록'과 **직접 연결되는** 뉴스 — 그 흐름의 새로운 진전을 보여주는 것.
3순위. 한국은행·금융위·금감원의 결정과 그 파급 — 금리, 가계부채, 자본규제.
4순위. 개별 금융사의 실적·전략 발표 중 **업계 전체에 시사점**이 있는 것.

후순위로 미루거나 아예 빼세요:
- 개별 종목의 주가 등락, 일일 시황, 환율 수치 그 자체 (정책적 함의가 있으면 예외)
- 한국 금융권과 연결고리가 없는 해외 기업·산업 뉴스
- 보도자료를 그대로 옮긴 홍보성 기사, 단순 행사 개최·시상 소식
- 부동산 시황 (단, 가계대출·DSR 규제와 연결되면 다룰 것)

애매하면 이렇게 자문하세요: **"면접관이 이걸 물어볼까? 이 뉴스를 알면 답이 달라지나?"**
...
## 3단계: 뉴스레터 작성
각 기사마다 핵심 수치와 배경, 의미(So What)를 담아 알차게 쓰세요.
기본적으로 위 요약문만으로 작성하세요. 그것만으로 의미를 쓸 수 없는 결정적인 기사가 있을 때에만
web_fetch로 본문을 읽으세요 (최대 {MAX_WEB_FETCHES}건, 대개 0~1건이면 충분합니다).

### 서식 — 아래 틀을 그대로 따르세요
독자는 매일 아침 같은 모양의 브리핑을 받아야 합니다. 줄 순서, 기호, 빈 줄까지 아래와 똑같이 맞추세요.
아래 내용은 서식을 보여주는 예시일 뿐이니, N 같은 자리표시는 실제 수치로 채우고 기사도 오늘 것으로 바꾸세요.

📬 금융 데일리 브리핑 — {today}

오늘의 요약
금융위가 가계대출 관리방안을 예고했고, 시중은행은 한도 축소로 선제 대응하고 있다.
하반기 규제 강도가 상반기보다 높아지는 흐름이다.

━━━━━━━━━━━━━━━━━━
1️⃣ 주요 시중은행 동향

▪️ KB금융, 주택담보대출 한도 축소 [가계부채 관리 강화]
   9월부터 주담대 한도를 연소득의 N배로 조정한다. 상반기 가계대출 잔액이 N조원 늘어
   당국 권고치를 넘어선 데 따른 조치다.
   💡 의미: 당국 압박이 직접 규제가 아니라 은행 자율 축소 형태로 옮겨가는 신호다.
...
❓ 예상 질문: 가계부채 관리 강화가 은행 수익성에 미치는 영향은?
🗂 답변 프레임:
   ① 현상 — 무엇이 어떻게 바뀌었는지 수치로
   ② 영향 — 이자이익과 건전성에 각각 어느 방향으로 작용하는지
   ③ 대응 — 은행들이 실제로 택하고 있는 전략
🔁 꼬리질문: 그렇다면 비이자이익 확대는 현실적으로 가능한가?

### 서식 규칙 (반드시 지킬 것)
- 첫 줄은 반드시 `📬 금융 데일리 브리핑 — {today}` 입니다. 그 앞에 아무것도 쓰지 마세요.
- 인사말·맺음말을 쓰지 마세요. "안녕하세요", "오늘도 좋은 하루 되세요", "도움이 되셨길" 전부 금지입니다.
...
- 기사 제목 끝의 [트렌드명]은 아래 4단계의 거시 트렌드 목록에서 고르고, 4단계에서 분류할 trend 값과
  반드시 일치시키세요. 어느 트렌드에도 해당하지 않는 기사(3️⃣ 채용 공고 등)에는 태그를 붙이지 마세요.
- 뉴스레터를 마친 뒤, 그 아래에 오늘 다룬 주제들을 아래 형식으로 정리하세요 (발송 전 자동 제거됨):
<topics>
주제1
주제2
...
...
## 4단계: 기사 분류 (웹사이트 아카이브용 — 발송 전 자동 제거됨)
...
def validate_newsletter(body: str, topics: list[str]) -> None:
    """발송 전 최종 점검. 두 종류로 나눠 다르게 처리합니다.

    problems — 뉴스레터가 망가졌다는 신호. 발송하지 않고 워크플로를 빨간 X로 끝냅니다.
        2026-08-16에 모델이 '다시 시도하겠습니다'만 반복한 응답이 그대로 발송된 적이
        있어서 추가했습니다. 정상 뉴스레터라면 항상 만족하는 조건들입니다.

    warnings — 표준 서식과 어긋났지만 읽는 데는 지장이 없는 것. 로그만 남기고 발송합니다.
        이모지 하나 때문에 그날 브리핑을 통째로 못 받는 손해가 더 크다고 보았습니다.
        Actions 로그에 며칠에 한 번, 어느 부분이 어긋나는지 쌓이므로,
        경고가 잦아지면 서식을 파이썬이 직접 조립하는 방식으로 옮기면 됩니다.
    """
    problems = []
    ...
    for warning in check_format(body):
        print(f"   ⚠️  서식: {warning}")


def check_format(body: str) -> list[str]:
    """표준 서식과 어긋난 부분을 찾아 사람이 읽을 수 있는 문장으로 돌려줍니다."""
    warnings = []
    lines = body.strip().splitlines() or [""]

    if not lines[0].strip().startswith(NEWSLETTER_START):
        warnings.append(f"첫 줄이 제목이 아닙니다 → {lines[0].strip()[:40]!r}")

    # 제목 바로 다음은 '오늘의 요약'이어야 합니다. 인사말이 끼면 여기서 걸립니다.
    if SUMMARY_MARKER not in "\n".join(lines[:4]):
        warnings.append(f"제목 다음에 '{SUMMARY_MARKER}'가 없습니다 (인사말이 끼었을 수 있음)")

    missing = [s for s in REQUIRED_SECTIONS if s not in body]
    positions = [body.index(s) for s in REQUIRED_SECTIONS if s in body]
    if missing:
        warnings.append("섹션이 빠졌습니다 → " + " ".join(missing))
    ...
    if stray:
        warnings.append("허용되지 않은 이모지 → " + " ".join(stray))

    if SITE_URL in body:
        warnings.append("본문에 사이트 주소가 있습니다 — 발송 시 자동으로 붙으므로 중복됩니다")

    for mark in ("**", "##"):
        if mark in body:
            warnings.append(f"마크다운 기호 '{mark}'가 남아 있습니다")
